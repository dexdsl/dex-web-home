(function () {
  ...
  function extractFilterValue(filterValue, fnName, unit) {
    if (!filterValue) return null;
    var unitSuffix = unit || "";
    var escapedUnit = unitSuffix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    var re = new RegExp(fnName + "\\(([-\\d.]+)" + escapedUnit + "\\)", "i");
    var match = String(filterValue).match(re);
    if (!match) return null;
    var n = parseFloat(match[1]);
    return isFinite(n) ? n : null;
  }

  function toFixedCssNumber(value) {
    var str = String(Math.round(value * 100) / 100);
    return str.replace(/\.0+$/, "").replace(/(\.\d*[1-9])0+$/, "$1");
  }

  function buildThickerGlassFilter(filterValue) {
    var blur = extractFilterValue(filterValue, "blur", "px");
    var saturate = extractFilterValue(filterValue, "saturate", "%");
    var contrast = extractFilterValue(filterValue, "contrast", "");
    var boostedBlur = blur != null ? Math.max(22, Math.min(34, blur + 6)) : 24;
    var boostedSaturate = saturate != null ? Math.max(185, Math.min(235, saturate + 24)) : 205;
    var boostedContrast = contrast != null ? Math.max(1.04, Math.min(1.16, contrast + 0.04)) : 1.06;
    return "saturate(" + Math.round(boostedSaturate) + "%) blur(" + toFixedCssNumber(boostedBlur) + "px) contrast(" + toFixedCssNumber(boostedContrast) + ")";
  }
  ...
  function syncAuthUiGlass(ui) {
    ...
    var thickerFilter = buildThickerGlassFilter(filter || webkitFilter || "saturate(180%) blur(18px)");
    ...
    if (thickerFilter) ui.style.setProperty("--dex-header-glass-filter", thickerFilter);
    if (thickerFilter) ui.style.setProperty("--dex-header-glass-webkit-filter", thickerFilter);
  }
  ...
  function ensureAuthUi() {
    ...
    if (!document.getElementById(styleId)) {
      ...
      style.id = styleId;
      style.textContent = ""
        + "#auth-ui{--dex-text:rgba(16,24,36,.92);--dex-text-muted:rgba(16,24,36,.72);--dex-glass-bg:var(--dex-header-glass-bg,linear-gradient(120deg,rgba(221,230,240,.36) 0%,rgba(191,208,224,.26) 55%,rgba(232,210,203,.24) 100%));--dex-glass-border:var(--dex-header-glass-border,rgba(255,255,255,.42));--dex-glass-border-strong:var(--dex-header-glass-border-strong,rgba(255,255,255,.62));--dex-glass-shadow:var(--dex-header-glass-shadow,0 16px 36px rgba(18,22,30,.22),inset 0 1px 0 rgba(255,255,255,.32));--dex-glass-filter:var(--dex-header-glass-filter,saturate(205%) blur(24px) contrast(1.06));--dex-glass-webkit-filter:var(--dex-header-glass-webkit-filter,saturate(205%) blur(24px) contrast(1.06));--dex-neutral-overlay:rgba(211,219,228,.22);--dex-neutral-overlay-strong:rgba(204,213,223,.28);--dex-menu-overlay:rgba(255,255,255,0);--dex-menu-overlay-strong:rgba(255,255,255,.04);--dex-grain-opacity:.08;--dex-text-shadow:none;--dex-radius:10px;--dex-avatar-radius:4px;--dex-nav-h:38px;--dex-space-2:var(--space-2,8px);--dex-space-3:var(--space-3,12px);position:relative;top:0;display:inline-flex;align-items:center;align-self:center;vertical-align:middle;line-height:1;overflow:visible;padding:0;margin:0;gap:var(--dex-space-2);font-family:inherit;color:var(--dex-text);transform:translateY(var(--dex-nav-offset-y,0px));}"
        ...
        + "#auth-ui-profile-toggle{position:relative;gap:0;border:1px solid var(--dex-glass-border);background:linear-gradient(0deg,var(--dex-neutral-overlay),var(--dex-neutral-overlay)),var(--dex-glass-bg);box-shadow:var(--dex-glass-shadow);width:auto;min-width:calc(var(--dex-nav-h,38px) + 22px);border-radius:var(--dex-radius);cursor:pointer;justify-content:flex-start;padding:2px 28px 2px 2px;backdrop-filter:var(--dex-glass-filter);-webkit-backdrop-filter:var(--dex-glass-webkit-filter);transition:border-color .22s ease,filter .22s ease,background .22s ease;overflow:hidden;}"
        + "#auth-ui-profile-toggle:hover{border-color:var(--dex-glass-border-strong);background:linear-gradient(0deg,var(--dex-neutral-overlay-strong),var(--dex-neutral-overlay-strong)),var(--dex-glass-bg);}"
        ...
        + "#auth-ui-dropdown{position:absolute;right:0;top:calc(100% + 10px);width:min(292px,calc(100vw - 20px));max-width:calc(100vw - 20px);max-height:min(70vh,420px);overflow:auto;border:1px solid var(--dex-glass-border);border-top-color:var(--dex-glass-border-strong);border-radius:calc(var(--dex-radius) + 2px);background:linear-gradient(0deg,var(--dex-menu-overlay-strong),var(--dex-menu-overlay-strong)),var(--dex-glass-bg);box-shadow:var(--dex-glass-shadow);padding:var(--dex-space-2);z-index:1200;opacity:0;transform:translateY(-6px) scale(.985);pointer-events:none;backdrop-filter:var(--dex-glass-filter);-webkit-backdrop-filter:var(--dex-glass-webkit-filter);transition:opacity .2s ease,transform .2s ease;}"
        ...
        + "#auth-ui .dex-menu-item:hover{transform:translateY(-1px);border-color:var(--dex-glass-border-strong);background:linear-gradient(0deg,var(--dex-menu-overlay-strong),var(--dex-menu-overlay-strong)),var(--dex-glass-bg);}"
        ...
        + "#auth-ui .dex-menu-item.is-logout,#auth-ui #auth-ui-logout{margin-bottom:0;color:var(--dex-text);background:linear-gradient(0deg,var(--dex-menu-overlay),var(--dex-menu-overlay)),var(--dex-glass-bg);border-color:var(--dex-glass-border);}"
        + "#auth-ui .dex-menu-item.is-logout .dex-menu-label,#auth-ui #auth-ui-logout .dex-menu-label{font-weight:700;}"
        + "#auth-ui .dex-menu-item.is-logout:hover,#auth-ui #auth-ui-logout:hover{border-color:var(--dex-glass-border-strong);background:linear-gradient(0deg,var(--dex-menu-overlay-strong),var(--dex-menu-overlay-strong)),var(--dex-glass-bg);}"
