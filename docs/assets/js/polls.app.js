(()=>{(()=>{if(typeof window=="undefined"||typeof document=="undefined"||window.__dxPollsAppLoaded)return;window.__dxPollsAppLoaded=!0;let k=120,$="dx-polls-app-style",L=12,M=8;function D(){if(document.getElementById($))return;let t=document.createElement("style");t.id=$,t.textContent=`
    ...
    `,document.head.appendChild(t)}function u(t){return String(t!=null?t:"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function N(t){let e=String(t||"/").replace(/\/+/g,"/");return e==="/"?"/":e.endsWith("/")?e.slice(0,-1):e}function _(){let t=window.DEX_API_BASE_URL||window.DEX_API_ORIGIN||"";return String(t||"https://dex-api.spring-fog-8edd.workers.dev").trim().replace(/\/$/,"")}function h(t){let e=Date.parse(String(t||""));return Number.isFinite(e)?e:null}function g(t){let e=h(t);if(!e)return"TBD";try{return new Date(e).toLocaleString(void 0,{year:"numeric",month:"short",day:"numeric"})}catch{return new Date(e).toISOString().slice(0,10)}}function A(t){let e=h(t);if(!e)return"Closing date TBD";let o=Date.now(),a=e-o;if(a<=0)return"Closed";let l=Math.floor(a/36e5),n=Math.floor(l/24),s=l%24;return n>0?`${n}d ${s}h left`:s>0?`${s}h left`:`${Math.max(1,Math.floor(a/6e4))}m left`}function z(t){return Array.isArray(t)?t.map(e=>String(e||"").trim()).filter(Boolean):typeof t=="string"?t.split("|").map(e=>e.trim()).filter(Boolean):[]}function b(t){let e=t&&typeof t=="object"?t:{};return{id:String(e.id||"").trim(),slug:String(e.slug||"").trim()||null,status:String(e.status||"").trim()||"draft",question:String(e.question||"").trim()||"Untitled poll",options:z(e.options),createdAt:String(e.createdAt||e.created_at||"").trim(),closeAt:String(e.closeAt||e.close_at||"").trim(),manualClose:!!(e.manualClose||e.manual_close),visibility:String(e.visibility||"public").trim()==="members"?"members":"public",closed:!!e.closed,locked:!!e.locked}}function R(t){let e={};if(!t||typeof t!="object")return e;for(let[o,a]of Object.entries(t)){let l=Number(a);!Number.isFinite(l)||l<0||(e[String(o)]=Math.floor(l))}return e}function B(t){let e=t&&typeof t=="object"?t:{},o=Array.isArray(e.counts)?e.counts.map(a=>Math.max(0,Number(a)||0)):R(e.counts);return{total:Math.max(0,Number(e.total)||0),counts:o,viewerVote:Number.isInteger(Number(e.viewerVote))?Number(e.viewerVote):null,closed:!!e.closed}}function q(t){return`/polls/${encodeURIComponent(t)}/`}function T(){let t=N(window.location.pathname||"/");if(t==="/polls"||t==="/polls/index.html")return{type:"list",pollId:null};if(t.startsWith("/polls/")){let o=t.slice(7).replace(/\/index\.html$/i,"").replace(/\/$/,"");return o?{type:"detail",pollId:decodeURIComponent(o)}:{type:"list",pollId:null}}return{type:"list",pollId:null}}function U(){return document.querySelector("[data-dx-polls-app]")||document.getElementById("dex-console")||document.getElementById("dx-polls-app")}function j(){return window.DEX_AUTH||window.dexAuth||null}async function y(){let t=j();if(!t)return{auth:null,authenticated:!1,token:null,user:null};try{typeof t.resolve=="function"?await t.resolve(2500):t.ready&&typeof t.ready.then=="function"&&await t.ready}catch{}let e=!1;try{typeof t.isAuthenticated=="function"&&(e=!!await t.isAuthenticated())}catch{}let o=null;if(e&&typeof t.getAccessToken=="function")try{o=await t.getAccessToken()}catch{o=null}let a=null;try{typeof t.getUser=="function"&&(a=await t.getUser())}catch{}return{auth:t,authenticated:e,token:o,user:a}}async function f(t,e={}){let o=_(),a=e.auth||"optional",l=e.authSnapshot||await y(),n={"content-type":"application/json"};if(l.token&&(n.authorization=`Bearer ${l.token}`),a==="required"&&!n.authorization)return{ok:!1,status:401,data:{error:"AUTH_REQUIRED"}};let s=await fetch(`${o}${t}`,{method:e.method||"GET",headers:n,body:e.body?JSON.stringify(e.body):void 0}),d=null;try{d=await s.json()}catch{d=null}return{ok:s.ok,status:s.status,data:d}}function S(t,e=1){if(Array.isArray(t))return{polls:t.map(b),page:e,pages:1,total:t.length};let o=t&&typeof t=="object"?t:{},l=[o.polls,o.items,o.data,o.rows].find(n=>Array.isArray(n))||[];return{polls:l.map(b),page:Math.max(1,Number(o.page)||e),pages:Math.max(1,Number(o.pages||o.totalPages)||1),total:Math.max(0,Number(o.total||o.count||l.length)||0)}}async function E(t){let e=performance.now()-t;e>=k||await new Promise(o=>setTimeout(o,k-e))}function v(t,e){t.setAttribute("data-dx-fetch-state",e),e==="loading"?t.setAttribute("aria-busy","true"):t.removeAttribute("aria-busy")}function O(t,e){t.innerHTML=`
    ...
    `}function C(t){if(!t||t.status==="closed"||t.manualClose||t.closed)return!0;let e=h(t.closeAt);return e?e<=Date.now():!1}function I(t,e){let o=C(t),a=t.visibility==="members"&&!e.authenticated,l=o?"Closed":t.status==="draft"?"Draft":"Open",n=o?`Closed ${g(t.closeAt)}`:`Closes ${g(t.closeAt)} (${A(t.closeAt)})`,s=t.visibility==="members"?'<span class="dx-poll-chip">Members</span>':'<span class="dx-poll-chip">Public</span>',d=a?`<button class="dx-poll-action" type="button" data-dx-poll-action="signin" data-dx-poll-id="${u(t.id)}">Sign in to unlock</button>`:`<a class="dx-poll-link" href="${u(q(t.id))}">View Poll</a>`;return`
    ...
    `}function H(t,e){let o=e.open.polls.length?e.open.polls.map(l=>I(l,e.auth)).join(""):'<p class="dx-polls-empty">No open polls right now.</p>',a=e.closed.polls.length?e.closed.polls.map(l=>I(l,e.auth)).join(""):'<p class="dx-polls-empty">No closed polls yet.</p>';t.innerHTML=`
      <section class="dx-polls-shell">
        <article class="dx-polls-panel">
          <h1 class="dx-polls-title">Dex Polls</h1>
    ...
    `}function V(t,e,o,a,l="idle"){let n=C(e)||!!o.closed,s=Array.isArray(o.counts)?o.counts:e.options.map((i,r)=>{var c;return Number(((c=o.counts)==null?void 0:c[String(r)])||0)}),d=e.options.map((i,r)=>{let c=Math.max(0,Number(s[r])||0),m=o.total>0?Math.round(c/o.total*100):0;return`
        <button type="button" class="dx-poll-option${o.viewerVote===r?" is-selected":""}" data-dx-poll-action="vote" data-dx-poll-option="${r}" ${n?"disabled":""}>
          ...
          <div class="dx-poll-bar"><div class="dx-poll-bar-fill" style="width:${m}%"></div></div>
          <div class="dx-poll-row-foot"><span>${c} votes</span><span>${m}%</span></div>
        </button>
      `}).join(""),p=e.visibility==="members"?'<span class="dx-poll-chip">Members</span>':'<span class="dx-poll-chip">Public</span>',x=l==="saving"?"Submitting vote\u2026":l==="saved"?"Vote saved":l==="error"?"Vote failed":"";t.innerHTML=`
      <section class="dx-polls-shell">
        <article class="dx-polls-panel dx-poll-detail" data-dx-motion="pagination">
          ...
          <p class="dx-polls-subtitle">${n?`Closed ${g(e.closeAt)}`:`Closes ${g(e.closeAt)} (${A(e.closeAt)})`}</p>
    ...
    `}async function w(t){let e=t&&t.auth;if(!(!e||typeof e.signIn!="function"))try{await e.signIn({returnTo:`${window.location.pathname}${window.location.search}${window.location.hash}`})}catch{}}async function G(t,e){let o=e,a=1;async function l(){let n=await f(`/polls?state=open&page=1&pageSize=${L}`,{auth:"optional",authSnapshot:o}),s=await f(`/polls?state=closed&page=${a}&pageSize=${M}`,{auth:"optional",authSnapshot:o});if(!n.ok||!s.ok)throw new Error("Failed to load poll lists");let d=S(n.data,1),p=S(s.data,a);a=p.page,H(t,{auth:o,open:d,closed:p}),t.querySelectorAll('[data-dx-poll-action="signin"]').forEach(r=>{r.addEventListener("click",async c=>{c.preventDefault(),await w(o)})});let x=t.querySelector('[data-dx-poll-action="closed-prev"]');x&&x.addEventListener("click",async()=>{a<=1||(a-=1,await l())});let i=t.querySelector('[data-dx-poll-action="closed-next"]');i&&i.addEventListener("click",async()=>{a+=1,await l()})}await l()}async function X(t,e,o){var x;let a=o,l="idle",n=await f(`/polls/${encodeURIComponent(e)}`,{auth:"optional",authSnapshot:a});if(n.status===403||n.status===401){F(t,e);let i=t.querySelector('[data-dx-poll-action="signin"]');i&&i.addEventListener("click",async()=>{await w(a)});return}if(!n.ok)throw new Error(`Unable to load poll ${e}`);let s=b(((x=n.data)==null?void 0:x.poll)||n.data);if(!s.id)throw new Error("Poll payload is missing id");async function d(){var r;let i=await f(`/polls/${encodeURIComponent(s.id)}/results`,{auth:"optional",authSnapshot:a});if(!i.ok)throw new Error("Unable to load poll results");return B(((r=i.data)==null?void 0:r.results)||i.data)}async function p(){let i=await d();V(t,s,i,a,l),t.querySelectorAll('[data-dx-poll-action="vote"]').forEach(r=>{r.addEventListener("click",async()=>{let c=Number(r.getAttribute("data-dx-poll-option"));if(!(!Number.isInteger(c)||c<0)){if(a=await y(),!a.authenticated){await w(a);return}l="saving";try{let m=await f(`/polls/${encodeURIComponent(s.id)}/vote`,{method:"POST",auth:"required",authSnapshot:a,body:{optionIndex:c}});if(!m.ok){if(m.status===409){l="error",await p();return}throw new Error("Vote failed")}l="saved",await p(),window.setTimeout(()=>{l="idle",p().catch(()=>{})},900)}catch{l="error",await p()}}})})}await p()}async function P(){let t=U();if(!t)return;D();let e=performance.now();v(t,"loading");try{let o=await y(),a=T();a.type==="detail"&&a.pollId?await X(t,a.pollId,o):await G(t,o),await E(e),v(t,"ready")}catch(o){console.error("[dx-polls] boot error",o),O(t,"Unable to load polls right now. Please try again."),await E(e),v(t,"error")}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{P().catch(()=>{})},{once:!0}):P().catch(()=>{})})();})();
